...
for Dz in [10,100,1000]:
    ...
    Cmatrix_euler=np.zeros(shape=(tmax/Dz,L/Dx))
    
    for j in range(0,J):
        Cmatrix_euler[0,j]=C[j]
        
    for t in range(1,tmax/Dz):
        for j in range(0,xmax/Dx):
            Cmatrix_euler[t,j]=Cmatrix_euler[t-1,j]-u0*(Cmatrix_euler[t-1,j]-Cmatrix_euler[t-1,j-1])*Dz/Dx
    
    if Dz==10:
        plt.plot(xvec,Cmatrix_euler[tmax/Dz-1], 'k-',linewidth=2)
    if Dz==100:
        plt.plot(xvec,Cmatrix_euler[tmax/Dz-1], 'g-',linewidth=2)
    if Dz==1000:
        plt.plot(xvec,Cmatrix_euler[tmax/Dz-1], 'b-',linewidth=2)
